...
export const TaskItem = ({
  ...
  return (
    <div
      className={`group relative flex flex-row bg-white rounded-2xl border-2 transition-all overflow-hidden border-slate-100 shadow-sm ${
        task.is_completed ? "opacity-60 grayscale-[0.2] bg-slate-50" : ""
      }`}
    >
      <div
        onClick={!isSupporter && !isClaimed ? handleLeftClick : undefined}
        className={`p-3 sm:p-5 pl-12 sm:pl-16 flex-1 min-w-0 relative z-10 flex flex-col ${task.is_completed ? "bg-slate-50" : "bg-white"} ${
      ...
      <div
        ...
        className={`p-3 sm:p-5 w-24 sm:w-36 shrink-0 flex flex-col items-center justify-center relative z-10 transition-colors ${
          ...
          canClaimPoints
            ? "cursor-pointer active:scale-[0.95] transition-all"
            : ""
        }`}
      >
        <span
        ...
        >
          {isClaimed ? "Received" : canClaimPoints ? "Tap to claim" : "Reward"}
        </span>

        <div className="flex flex-col items-center">
          <div
            className={`flex items-baseline ${
              isClaimed ? "text-slate-300" : "text-sky-500"
            }`}
          >
